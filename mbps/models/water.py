...
class Water(Module):
    ...
    p : dictionary of floats
        ...
        'krf1'        [-] Rootfraction layer 1 assumed to be constant apperently
        'krf2'        [-] Rootfraction layer 2 assumed to be constant apperently
        'krf3'        [-] Rootfraction layer 3 assumed to be constant apperently
    ...
    * Mohtar, R.H., Buckmaster, D.R., & Fales, S.L. (1997),
    ...
    def diff(self, _t, _x0):
        ...
        L_i = _x0[0:3]  # [mm] Water in soil layer i
        # -- Parameters
        ...
        WAIc = self.p['WAIc']  # 0.75 [-] Critical water value for water availability index
        ...
        fc_i = np.array([theta_fc1, theta_fc2, theta_fc3])  # [-] Field capacity of soil layer i
        ...
        pwp_i = np.array([theta_pwp1, theta_pwp2, theta_pwp3])  # [-] Permanent wilting point of soil layer i
        ...
        k_rfi = np.array([krf1, krf2, krf3])  # [-] Rootfraction layer i
        ...
        # -----------------Effective precipitation below-----------------
        f_Ru = np.where(_f_prc > 0.2 * S, (_f_prc - 0.2 * S) ** 2 / (_f_prc + 0.8 * S), 0)  # [mm d-1] Surface runoff
        f_Pe = _f_prc - f_Ru  # [mm d-1] Effective precipitation

        # -----------------Effective precipitation above-----------------
        # -----------------Transpiration below-----------------
        delta = (5304 / ((T_mean + 273) ** 2)) * np.exp(21.3 - (5304 / (T_mean + 273)))
        rn = 0.408 * _I_glb * (1 - alb)
        ET0 = alpha * rn * delta / (delta + gamma)
        ETp = kcrop * ET0
        k_Tr = 1 - np.exp(-0.6 * _LAI)
        T_p = k_Tr * ETp
        WAI_i = np.where((L_i - pwp_i) / (fc_i - pwp_i) > 0, 0,
                         (L_i - pwp_i) / (fc_i - pwp_i))  # [-] Water availability index
        k_ra = 0.0408 * np.exp(0.19 * _T)  # [-] Root activity
        k_rti = np.where(WAI_i < WAIc, WAI_i / WAIc, 1)  # [-] Restriction of transpiration by soil

        f_Tri = k_rti * k_ra * k_rfi * T_p
        # -----------------Transpiration above-----------------
        # -----------------Evaporation Below-----------------
        if (ETp - T_p) > ETp * (1 - mlc):
            E_p = ETp * (1 - mlc)  # don't write ETp(1-mlc)
        else:
            E_p = ETp - T_p
        f_Ev = E_p / np.sqrt(DSD)
        # -----------------Evaporation above-----------------
        # -----------------Drainage Below-----------------

        f_Dri = np.where(L_i > fc_i, L_i - fc_i, 0) / dt
        # -----------------Drainage above-----------------
